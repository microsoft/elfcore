...
impl CoreDumpBuilder {
    ...
    pub fn write<T: Write>(mut self, writer: T) -> Result<usize, CoreError> {
        ...
        let memory_reader = if process_vm_readv_works() {
            ...
            Box::new(FastMemoryReader::new(self.pv.pid)?) as Box<dyn ReadProcessMemory>
        } else {
            tracing::info!("Using the slow process memory read on this system");
            Box::new(SlowMemoryReader::new(self.pv.pid)?) as Box<dyn ReadProcessMemory>
        };

        let note_sizes = self.get_elf_notes_sizes()?;

        total_written += self.write_elf_header(&mut writer)?;
        total_written += writer.align_position(ELF_HEADER_ALIGN)?;
        total_written += self.write_program_headers(&mut writer, &note_sizes)?;
        total_written += writer.align_position(ELF_HEADER_ALIGN)?;
        total_written += self.write_elf_notes(&mut writer, &note_sizes)?;
        total_written += writer.align_position(self.pv.page_size)?;
        total_written += self.write_va_regions(&mut writer, memory_reader)?;
    ...
    fn write_elf_header<T: Write>(
        ...
        let elf_header = Elf64_Ehdr {
            ...
            e_phnum: 1 + self.pv.va_regions.len() as u16, // PT_NOTE and VA regions
            e_shentsize: 0,
            e_entry: 0,
            e_shoff: 0,
            e_flags: 0,
    ...
    fn write_program_headers<T: Write>(
        ...
        let phdr_size = std::mem::size_of::<Elf64_Phdr>() * (self.pv.va_regions.len() + 1);
        let ehdr_size = std::mem::size_of::<Elf64_Ehdr>();
        let data_offset =
            round_up(ehdr_size, ELF_HEADER_ALIGN) + round_up(phdr_size, ELF_HEADER_ALIGN);
        ...
        let mut current_offset =
            round_up(data_offset + note_sizes.total_note_size, self.pv.page_size);

        for region in &self.pv.va_regions {
            let mut seg_header = Elf64_Phdr {
                ...
                p_align: self.pv.page_size as u64,
            };
    ...
    fn write_process_info_note<T: Write>(
        ...
        for thread_view in &self.pv.threads {
            if thread_view.tid == self.pv.pid {
    ...
    fn write_process_status_notes<T: Write>(
        ...
        for thread_view in &self.pv.threads {
            ...
            let mut written =
                write_elf_note(writer, NT_PRSTATUS, NOTE_NAME_CORE, status.as_bytes())?;
    ...
    fn write_aux_vector_note<T: Write>(
        ...
        let written = write_elf_note(
            writer,
            NT_AUXV,
            NOTE_NAME_CORE,
            self.pv.aux_vector.as_bytes(),
        )?;
    ...
    fn write_mapped_files_note<T: Write>(
        ...
        let mut data: Vec<u8> = Vec::with_capacity(self.pv.page_size);
        ...
        for mapped_file in &self.pv.mapped_files {
    ...
    fn write_elf_notes<T: Write>(
        ...
        if note_sizes.process_info != 0 {
            written = self.write_process_info_note(writer)?;
        ...
        if note_sizes.process_status != 0 {
            written = self.write_process_status_notes(writer)?;
        ...
        if note_sizes.aux_vector != 0 {
            written = self.write_aux_vector_note(writer)?;
        ...
        if note_sizes.mapped_files != 0 {
            written = self.write_mapped_files_note(writer)?;
        ...
        if note_sizes.custom != 0 {
            written = self.write_custom_notes(writer)?;
    ...
    fn write_va_regions<T: Write>(
        ...
        for va_region in &self.pv.va_regions {
            let dumped = self.write_va_region(writer, va_region, &mut memory_reader)?;
    ...
    fn get_elf_notes_sizes(&self) -> Result<NoteSizes, CoreError> {
        let header_and_name =
        ...
        let process_status = one_thread_status * self.pv.threads.len();
        let aux_vector =
            header_and_name + self.pv.aux_vector.len() * std::mem::size_of::<Elf64_Auxv>();
